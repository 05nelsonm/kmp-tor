...
class NotificationStateUnitTest {
    ...
    fun givenState_whenToString_thenIsAsExpected() {
        ...
        assertEquals("""
            NotificationState[fid=abcd…5678]: [
                actions: [
                    NewIdentity
                ]
        ...
        val stateNoActions = state.copy(actions = emptySet())
        assertTrue(stateNoActions.toString().contains("    actions: []"))
    }
}
